...
class R2Image {
 public:
  ...
  R2Image(const char *filename, std::vector<int> givenLocs3DX, std::vector<int> givenLocs3DY, std::vector<int> givenLocs3DZ); //for frames
  ...
  //Marker2D* Harris(double sigma);
  void Sharpen();

  // further operations
  void trackMarkersOntoOtherImage(Marker2D *originalMarkers, R2Image *otherImage, Marker2D *newMarkers);
  int RANDSAC(int maxIteration, int threshold, Marker2D *originalFeatures, Marker2D *matchedFeatures, double **HMatrix);
  double** calcHomographyMatrix(int *input, int * output, int size);
